...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 23 10:16:17 2022
...
import functools
import threading
from helpers.message_parser import parse_validate_json, InvalidMessageException
...
class EventListener:
    ...
    def do_work(self, channel, delivery_tag, body):
        ...
        # Start the transfer
        try:
            src_file = message['source_file']
            src_user = message['source_user']
            src_passwd = message['source_password']
            dst_passwd = message['destination_password']
            dst_user = message['destination_user']
            dst_file = message['destination_file']
            src_host = message['source_host']
            dst_host = message['destination_host']
            src_dir = message['source_path']
            dst_dir = message['destination_path']
            move = message['move']
            in_path = os.path.join(src_dir, src_file)
            out_path = os.path.join(dst_dir, dst_file)
            Fxp(src_host, src_user, src_passwd,
